...
import type { Query, Position, QueryTerminalSettingsTab, ApplicantCard, Applicant, SearchSettings, ApplicantRow } from "@/types";
...
type QueryTerminal = {
    ...
    setTableLoading: React.Dispatch<React.SetStateAction<{status: boolean, color: string, text: string}>>;
    ...
    setSettingsTab: React.Dispatch<React.SetStateAction<QueryTerminalSettingsTab>>;
}

const QueryTerminalContext = createContext<QueryTerminal | undefined>(undefined)
...
export const useQueryTerminal = () => {
  ...
  if (!context) throw new Error("use useQueryTerminal within a QueryTerminalProvider")

  return context
}

export const QueryTerminalManager = ({ children }: { children: React.ReactNode }) => {
    ...
    const [searchSettings, setSearchSettings] = useState<SearchSettings>({deepTopK: 10, flashTopK: 20, deepWeight: 0.5, flash: false});

    const [tagText, setTagText] = useState<string>("");

    const [applicantCard, setApplicantCard] = useState<ApplicantCard>({display: false});
    ...
    const [positions, setPositions] = useState<Position[]>([{id: 1, name: "General Application", status: "open"}]);
    ...
    const [settingsTab, setSettingsTab] = useState<QueryTerminalSettingsTab>("filters");
    return (
