...
export function RecentQueries() {
  const { recentQueries, setRecentQueries, recentQueriesLoading, setRecentQueriesLoading, setSavedQueries } = useQueries();
          ...
          <div className="relative flex items-center gap-2 justify-end pr-unit-2">
            <Tooltip content="Save Query" color={"primary"} delay={400} closeDelay={600}>
              <Button isIconOnly size="sm" variant="light"
                onPress = {async () => {
                    setSavedQueries((prev) => {return [{id: item.id, tags: item.tags, filter: item.filter}, ...prev]});
    ...
    <div className="flex flex-col h-full">
      <div className="flex-auto h-full">
        <Table isStriped aria-label="Example table with custom cells" className="flex-auto h-full">
          ...
          <TableBody emptyContent={recentQueriesLoading ? " " : "No recent queries found."} items={recentQueries} isLoading={recentQueriesLoading} loadingContent={<Spinner className="h-full w-full bg-default-50/75" label="Loading recent queries..." color="warning" labelColor="warning" />}>
            {(item) => (
