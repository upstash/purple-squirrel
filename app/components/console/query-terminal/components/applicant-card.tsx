...
function checkWebsiteUrl(url: string) {
  const regex =
    /^(https?:\/\/)?([A-Za-z0-9-]+\.)?[A-Za-z0-9-]+\.[A-Za-z0-9-]+(\/\S*)?$/;
  return regex.test(url);
}
              ...
              <div className="flex-initial flex flex-col items-center gap-2">
                ...
                {applicantWebsite && checkWebsiteUrl(applicantWebsite) ? (
                  <Button
                    isIconOnly
                    isExternal
                    href={`/external-ref?externalLink=${applicantWebsite}`}
                    as={Link}
                  >
                ...
                <Button
                  ...
                  onPress={async () => {
                    await fetch(`/api/console/delete-applicants`, {
                      ...
                      body: JSON.stringify({
                        applicants: [
                          { id: applicantID, positionId: applicantPositionID },
                        ],
                      }),
                    });
          ...
          <div className="flex gap-2 h-full">
            <div className="flex-auto flex flex-col gap-2 h-full">
              <div className="flex-[1_1_0%] flex gap-2 w-full">
                <div className="flex-[1_1_0%] bg-default-100 rounded-medium py-1 px-3 h-10"></div>
                <div className="flex-[1_1_0%] bg-default-100 rounded-medium py-1 px-3 h-10"></div>
                <div className="flex-[10_1_0%] bg-default-100 rounded-medium py-1 px-3 h-10"></div>
              </div>
              <div className="flex-[1_1_0%] flex gap-2 items-end w-full">
                <div className="flex-auto bg-default-100 rounded-medium py-1 px-3 h-10"></div>
              </div>
              <div className="flex-[1_1_0%] flex gap-2 items-end w-full">
                <div className="flex-auto bg-default-100 rounded-medium py-1 px-3 h-10"></div>
              </div>
...
export function ApplicantCard() {
  ...
  return (
    <div className="h-full">
      {applicantCard.display ? <InfoCard /> : <EmptyCard />}
    </div>
  );
}
