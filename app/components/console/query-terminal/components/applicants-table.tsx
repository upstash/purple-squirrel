...
const getColumnValue = (
  ...
  switch (columnKey) {
    ...
    case "position":
      return (
        positions.find((position) => position.id === applicant.positionId)
          ?.name || "Unknown"
      );
    case "location":
      return applicant.countryCode
...
export function ApplicantsTable() {
  ...
  const headerColumns = useMemo(() => {
    return columns.filter((column) =>
      ["score", "name", "location", "stars", "status", "actions"].includes(
        column.uid
      )
    );
  }, []);
  ...
  const renderCell = useCallback(
    (applicant: ApplicantRow, columnKey: Key) => {
      ...
      switch (columnKey) {
          ...
          return (
            <div>
              <p
                className={
                  applicantScore > 89
                    ? "text-bold text-lg bg-gradient-to-r from-secondary to-secondary-400 bg-clip-text text-transparent w-min"
                    : applicantScore > 79
                    ? "text-bold text-lg bg-gradient-to-r from-success to-success-300 bg-clip-text text-transparent w-min"
                    : applicantScore > 49
                    ? "text-bold text-lg bg-gradient-to-r from-warning to-warning-300 bg-clip-text text-transparent w-min"
                    : "text-bold text-lg bg-gradient-to-r from-danger to-danger-300 bg-clip-text text-transparent w-min"
                }
              >
                {Math.round(applicantScore)}
              </p>
            </div>
          );
        case "name":
        ...
        case "id":
          return <p className="text-bold text-sm capitalize">{applicantID}</p>;
        case "actions":
          return (
                ...
                <Button
                  ...
                  onPress={async () => {
                    ...
                    await fetch(`/api/console/delete-applicants`, {
                      ...
                      body: JSON.stringify({
                        applicants: [
                          { id: applicantID, positionId: applicant.positionId },
                        ],
                      }),
                    });
    ...
    return (
      <div className="py-2 px-2 flex items-center">
        <div className="flex-1 flex items-center gap-2">
          <Tooltip
            content="Delete Selected Applicants"
            color={"danger"}
            delay={400}
            closeDelay={600}
          >
            <Button
              ...
              onPress={async () => {
                ...
                if (
                  applicantCard.display &&
                  (selectedKeys === "all" ||
                    new Set(selectedKeys).has(applicantCard.id))
                ) {
                  setApplicantCard((prev) => ({ display: false }));
                }
                setTableLoading((prev) => {
          ...
          <Dropdown className="min-w-0 w-fit">
            <DropdownTrigger>
              ...
              >
                <Tooltip
                  content="Change Status of Selected Applicants"
                  color={"warning"}
                  delay={400}
                  closeDelay={600}
                >
            ...
            <CSVLink
              data={applicants.map((applicant) => {
                return {
                  ...
                  LOCATION: applicant.countryCode
                    ? LOCATION_LOOKUP[applicant.countryCode]
                    : "Unknown",
                  STATUS:
                    applicant.status === "newApply"
                      ? "New"
  ...
  return (
    ...
    >
      <TableHeader columns={headerColumns}>
        {(column) => (
          <TableColumn
            ...
            className={
              column.uid === "actions" ? "text-end pr-unit-10" : undefined
            }
          >
            {column.name}
          </TableColumn>
      ...
      <TableBody
        emptyContent={
          tableLoading.status ? " " : "Run a query to find applicants..."
        }
        items={sortedItems}
        isLoading={tableLoading.status}
        loadingContent={
          <Spinner
            className="h-full w-full bg-default-50/75"
            label={tableLoading.text}
            color={
              isNextUIColor(tableLoading.color) ? tableLoading.color : "default"
            }
            labelColor={
              isNextUIColor(tableLoading.color)
                ? tableLoading.color === "default"
                  ? "foreground"
                  : tableLoading.color
                : "foreground"
            }
