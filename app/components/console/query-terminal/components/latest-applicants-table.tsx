...
export function LatestApplicantsTable() {
  ...
  const headerColumns = useMemo(() => {
    return columns.filter((column) =>
      ["name", "position", "location", "stars", "status", "actions"].includes(
        column.uid
      )
    );
  }, []);
  ...
  const filteredItems = useMemo(() => {
    let filteredApplicants = [...latestApplicants];
  ...
  const renderCell = useCallback(
    ...
    [setLatestApplicants, setTableLoading, applicantCard, setApplicantCard, positions]
  );

  const handleRowAction = useCallback(
    async (key: Key) => {
      const applicant = latestApplicants.find((applicant) => applicant.id === parseInt(key as string));
      if (!applicant) {
        return;
      }
      setApplicantCard((prev) => ({ display: true, ...applicant }));
    },
    [latestApplicants, setApplicantCard]
  );
      ...
      <div className="py-2 px-2 flex items-center">
        <div className="flex-1 flex items-center gap-2">
          ...
          <Dropdown className="min-w-0 w-fit">
            ...
            <DropdownMenu
              ...
              onSelectionChange={async (keys) => {
                ...
                if (selectedKeys === "all") {
                  newSelectedKeys = latestApplicants.map((applicant) => applicant.id);
                } else {
          ...
          {latestApplicants.length > 0 ? (
            <CSVLink
              data={latestApplicants.map((applicant) => {
                return {
                  ID: applicant.id,
              ...
              filename={"latest-applicants.csv"}
            >
              CSV
            </CSVLink>
          ) : (
