...
export function UserTable() {
    const { users, setUsers, usersLoading, setUsersLoading, userSearchText, setUserSearchText, roleFilter, setRoleFilter, userRowsPerPage, setUserRowsPerPage, userTablePage, setUserTablePage, selectedUserKeys, setSelectedUserKeys } = useAdmin();
  ...
  const filteredItems = useMemo(() => {
    ...
    if (roleFilter !== "all" && Array.from(roleFilter).length !== roleOptions.length) {
        filteredUsers = filteredUsers.filter((user) =>
          ...
          (user.publicMetadata.role === "user" && Array.from(roleFilter).includes("User")) ||
          (user.publicMetadata.role === "recruiter" && Array.from(roleFilter).includes("Recruiter")) ||
          (user.publicMetadata.role === "admin" && Array.from(roleFilter).includes("Admin")) :
  ...
  const renderCell = useCallback((user: User, columnKey: Key) => {
    switch (columnKey) {
      ...
      case "role":
        const userRole = (user.publicMetadata.role as Role || undefined) || "user";
        return (
            <Dropdown className="min-w-0 w-fit">
                <DropdownTrigger>
                    <button>
                        <Chip className="capitalize" color={isRole(userRole) ? getRoleColor(userRole) : "default"} size="sm" variant="solid" endContent={<ArrowDropDownOutlinedIcon/>}>
                            {userRole}
                        </Chip>
                    </button>
                </DropdownTrigger>
                              ...
                              setUsers((prev: User[]) => {
                                  return prev.map((u) => {
                                      if (u.id === user.id && u.publicMetadata) {
                                          return { ...u, publicMetadata: { ...u.publicMetadata, role: chosenRole } } as unknown as User;
                                      }
                                      return u;
                                  });
                              });
                ...
                >
                    {roleArray.map((role) => 
                        <DropdownItem key={role} textValue={role}>
                            <Chip className="capitalize" color={isRole(role) ? getRoleColor(role) : "default"} size="sm" variant="solid">
                                {role}
                            </Chip>
                        </DropdownItem>
                    )}
              ...
              >
                {roleOptions.map((role) => (
                  <DropdownItem key={role.uid} className={role.uid === "All" ? "capitalize text-danger" : "capitalize"} variant="bordered">
                    {<Chip color={isRole(role.uid) ? getRoleColor(role.uid) : "default"} size="sm">{capitalize(role.name)}</Chip>}
                  </DropdownItem>
                ))}
              </DropdownMenu>
            </Dropdown>
      ...
      <div className="py-2 px-2 flex items-center">
        <div className="flex-1 flex items-center gap-2">
          <Tooltip content="Delete Selected Users" color={"danger"} delay={400} closeDelay={600}>
            <Button isIconOnly variant="light" aria-label="Delete Applicant" size="sm"
              onPress={async () => {
                if (!selectedUserKeys || (selectedUserKeys !== "all" && (new Set(selectedUserKeys).size === 0))) {
                  return;
                }
                setUsersLoading(true);
                const res = await fetch(`/api/users/delete-users`, {
                ...
                if (res.status !== 200) {
                  ...
                  setUsers((prev) => {
                      return prev.filter((u) => !(new Set(selectedUserKeys)).has(u.id));
                  });
          ...
          <Dropdown className="min-w-0 w-fit">
            ...
            <DropdownMenu 
                ...
                onSelectionChange={
                    async (keys) => {
                        if (!keys || keys === "all" || keys.size !== 1 || !selectedUserKeys || (new Set(selectedUserKeys)).size === 0) {
                            return;
                        }
                        const chosenRole = Array.from(keys)[0];
                        ...
                        if (res.status !== 200) {
                          ...
                          setUsers((prev) => {
                              return prev.map((u) => {
                                  if (selectedUserKeys === "all" || (new Set(selectedUserKeys)).has(u.id)) {
                                    return {...u, publicMetadata: {...u.publicMetadata, role: chosenRole}} as unknown as User;
                                  }
                                  return u;
                              });
            ...
            >
                {roleArray.map((key) => 
                    <DropdownItem key={key} textValue={key}>
                        <Chip className="capitalize" color={isRole(key) ? getRoleColor(key) : "default"} size="sm" variant="solid">
                            {key}
                        </Chip>
                    </DropdownItem>
                )}
          ...
          <span className="whitespace-nowrap w-[30%] text-small text-default-40 ml-2">
            {selectedUserKeys === "all"
              ...
              : (selectedUserKeys === undefined 
                ? "No items selected"
                : `${(new Set(selectedUserKeys)).size} of ${filteredItems.length} items`
              ) }
          </span>
        </div>
