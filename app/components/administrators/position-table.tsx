...
export function PositionTable() {
    const { positions, setPositions, positionsLoading, setPositionsLoading, positionSearchText, setPositionSearchText, statusFilter, setStatusFilter, rowsPerPage, setRowsPerPage, tablePage, setTablePage, selectedKeys, setSelectedKeys } = useAdmin();
  ...
  const filteredItems = useMemo(() => {
    ...
    if (statusFilter !== "all" && Array.from(statusFilter).length !== statusOptions.length) {
      filteredPositions = filteredPositions.filter((position) =>
        ...
        Array.from(statusFilter).includes("Open") : Array.from(statusFilter).includes("Closed")
      );
    }
  ...
  const renderCell = useCallback((position: Position, columnKey: Key) => {
    switch (columnKey) {
      case "id":
        return (
                            ...
                            if (res.status !== 200) {
                              ...
                              setPositions((prev) => {
                                return prev.map((pair) => {
                                    ...
                                    if (pair.id === position.id && isPositionStatus(newStatus)) {
              ...
              >
                {statusOptions.map((status) => (
                  <DropdownItem key={status.uid} className={status.uid === "All" ? "capitalize text-danger" : "capitalize"} variant="bordered">
                    {<Chip color={isPositionStatus(status.uid) ? getPositionStatusColor(status.uid) : "default"} size="sm">{capitalize(status.name)}</Chip>}
                  </DropdownItem>
                ))}
              </DropdownMenu>
            </Dropdown>
        ...
        <div className="flex-1 flex items-center gap-2">
          <Tooltip content="Delete Selected Positions" color={"danger"} delay={400} closeDelay={600}>
            <Button isIconOnly variant="light" aria-label="Delete Position" size="sm"
              onPress={async () => {
                if (!selectedKeys || (selectedKeys !== "all" && (new Set(selectedKeys)).size === 0)) {
                  return;
                }
                setPositionsLoading((prev) => {return true;});
                const res = await fetch(`/api/positions/delete-positions`, {
                  ...
                  body: JSON.stringify({positions: positions.filter((position) => {return (new Set(selectedKeys)).has(`${position.id}`);})}),
                });
                if (res.status !== 200) {
                  console.error("Failed to delete positions");
                } else {
                  setPositions((prev) => {return prev.filter((position) => {return !(new Set(selectedKeys)).has(`${position.id}`) || position.id === 1;});});
            ...
            <DropdownMenu 
                ...
                onSelectionChange={
                    async (keys) => {
                        ...
                        if (!keys || keys.size !== 1 || (new Set(selectedKeys)).size === 0) {
                          return;
                        }
                        ...
                        setPositionsLoading((prev) => {return true;});
                        let changePositions = (selectedKeys === "all") ? positions : positions.filter((position) => {return (new Set(selectedKeys)).has(`${position.id}`);});
                        ...
                        if (res.status !== 200) {
                          ...
                          setPositions((prev) => {
                            return prev.map((pair) => {
                              if ((selectedKeys === "all" || (new Set(selectedKeys)).has(`${pair.id}`)) && (pair.id !== 1)) {
            ...
            >
                {statusArray.map((key) => 
                    <DropdownItem key={key} textValue={key === "newApply" ? "new" : key}>
                        <Chip className="capitalize" color={isPositionStatus(key) ? getPositionStatusColor(key) : "default"} size="sm" variant="solid">
